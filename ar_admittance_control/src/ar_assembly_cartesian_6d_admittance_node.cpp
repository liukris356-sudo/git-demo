 ...
 public:
  AssemblyCartesian6DAdmittanceNode()
    ...
    admittance_.deadband = ar_admittance_control::vector6(
        ...
        "deadband");
    admittance_.max_position = ar_admittance_control::vector6(
        declare_parameter<std::vector<double>>(
            "max_offset",
            {0.0003, 0.0003, 0.0002, 0.00349066, 0.00349066, 0.00174533}),
        "max_offset");
    ...
    phase_approach_axes_ = phaseMask("phase_approach_axes",
                                    {0, 0, 0, 0, 0, 0});
    phase_lip_axes_ = phaseMask("phase_lip_axes", {1, 1, 0, 1, 1, 0});
    phase_release_axes_ = phaseMask("phase_release_axes", {1, 1, 0, 1, 1, 0});
    phase_insert_axes_ = phaseMask("phase_insert_axes", {1, 1, 0, 1, 1, 0});
    phase_flatten_axes_ = phaseMask("phase_flatten_axes", {1, 1, 0, 1, 1, 0});
    phase_seat_axes_ = phaseMask("phase_seat_axes", {1, 1, 0, 1, 1, 0});

    normal_center_other_ = ar_admittance_control::vector6(
        declare_parameter<std::vector<double>>(
            "normal_wrench_center_other", {0, 0, 0, 0, 0, 0}),
        "normal_wrench_center_other");
    normal_half_width_other_ = ar_admittance_control::vector6(
        declare_parameter<std::vector<double>>(
            "normal_wrench_half_width_other",
            {0.5, 0.7, 30.0, 0.05, 0.05, 1.25}),
        "normal_wrench_half_width_other");
  ...
  int run() {
    ...
    NominalTrajectory trajectory(points.at("P_SAFE"),
                                 points.at("P_EDGE_IN"), trajectory_error_,
                                 speed_scale_);
 ...
 private:
  ...
  void validate() const {
    ...
    if (points_file_.empty() ||
        trajectory_error_.head<3>().cwiseAbs().maxCoeff() > 0.0005 ||
        trajectory_error_.tail<3>().cwiseAbs().maxCoeff() > 0.5 * kDeg ||
        speed_scale_ <= 0.0 || speed_scale_ > 1.0 || !signs_valid ||
        soft_limit_margin_deg_ < 1.0 || soft_limit_margin_deg_ > 10.0 ||
        !normal_center_other_.allFinite() ||
        !normal_half_width_other_.allFinite() ||
        (normal_half_width_other_.array() <= 0.0).any() ||
        admittance_.max_position.head<3>().maxCoeff() > 0.0005 ||
        admittance_.max_position.tail<3>().maxCoeff() > 0.5 * kDeg ||
  ...
  void printPlan(const NominalTrajectory &trajectory) const {
    static const std::array<const char *, 4> labels{
        "P_SAFE(no error)", "P_BACK_LOW(+6D error)",
        "P_EDGE_IN(+6D error)", "P8(+6D error)"};
    ...
    RCLCPP_WARN(
        get_logger(),
        "Phase-aware profile is provisional. The node computes all 6 axes, "
        "but the YAML phase masks decide which axes may move. Seat "
  ...
  int runActive(const std::map<std::string, TaughtPoint> &points,
    ...
    try {
      ...
      std::cout
          ...
          << "Normal phase wrench is removed before correction.\n"
          << "Translation error [mm]: " << std::showpos
      ...
      std::function<rokae::CartesianPosition()> callback = [&]() {
        ...
        Vector6d center = normal_center_other_;
        Vector6d half_width = normal_half_width_other_;
        center[0] = interpolateProfile(
            profile_progress_, normal_fx_center_n_, progress);
        half_width[0] = interpolateProfile(
            profile_progress_, normal_fx_half_width_n_, progress);
        Vector6d excess = Vector6d::Zero();
        for (Eigen::Index i = 0; i < 6; ++i) {
          const double low = center[i] - half_width[i];
          const double high = center[i] + half_width[i];
        ...
        for (Eigen::Index i = 0; i < 6; ++i) {
          if (axes[i] > 0.5 &&
              std::abs(admittance_.position[i]) >=
                  0.999 * admittance_.max_position[i] &&
              std::abs(excess[i]) > admittance_.deadband[i]) {
            saturated_with_excess = true;
          }
        }
        saturated_cycles = saturated_with_excess
                               ? saturated_cycles + 1
                               : 0;
      ...
      RCLCPP_WARN(
          get_logger(),
          "ARMED: phase-aware 6D correction, maximum xyz offsets "
          "[%.3f %.3f %.3f] mm and rotation offsets "
          "[%.3f %.3f %.3f] deg; duration %.1f s.",
          admittance_.max_position[0] * 1000.0,
          admittance_.max_position[1] * 1000.0,
          admittance_.max_position[2] * 1000.0,
          admittance_.max_position[3] / kDeg,
          admittance_.max_position[4] / kDeg,
          admittance_.max_position[5] / kDeg, trajectory.duration());
      ...
      RCLCPP_INFO(
          ...
          "seat force=%+.2f N, seat=%s, max force=%.2f N, "
          "max torque=%.3f Nm",
  ...
  Vector6d normal_center_other_{Vector6d::Zero()};
  Vector6d normal_half_width_other_{Vector6d::Ones()};
