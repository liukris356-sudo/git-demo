...
一键启动时，如果传感器驱动连接失败或退出，监视器也会自动退出。若拔掉传感器后仍能看到数据，请检查是否有旧节点仍在发布：

```bash
ros2 node list
ros2 topic info /force_sensor/force --verbose
```
